...
One of the most common ways to model large neuronal networks is to use a simplification called a firing rate model. It is equivalent to a mean field approach where rather than record the spiking trains of every single neuron, one tracks the averaged behavior of the spike rates of groups of neurons. This transition from a microscopic description to a mesoscopic %TODO change mesoscopic to macro ?
...
%TODO Ger00,ChiGra07,ChiGra08,SchDeg17} HOW ?? 
...
%TODO ALL THE MODIFICATION  ASK HOW TO REORGANISZE AND SEPARAtTE TIME DEPENDENT INPUT ....
...
%and the double sum runs over all firing time $t^f$ of all neuron $j$ in the population.
...
%The  eigenfunction expansion is a method applied on partial differential equation. The main idea is identify an operator and expand the equation in terms of the operator eigenfunction times time dependent coefficients. 
...
%Where the evolution operator $\mathcal{L}$  L is a linear operator given a fixed history of the population activity A(t), s<t. 
...
To lighten the notation, we will omit the dependence on $h$ (the time dependent potential) of the hazard rate $\rho(\tau)$, The ISI distribution $P(\tau)$, the survivor function $S(\tau)$, and of the set of eigenfunctions $\{\ket{\phi_n}\}$ and eigenvalues $\{\lambda_n\}$.\\ \\%TODO keep \\ ? 
